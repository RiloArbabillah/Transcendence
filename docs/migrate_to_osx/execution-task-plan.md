...
- Slice berikutnya menutup satu seam object-reference aktif yang masih tersisa di jalur effect hit-parameter creation:
  - `Mammoth/TSE/CCreatePainterCtx.cpp` tidak lagi membungkus `CSpaceObject *` mentah ke `CCodeChain::CreateDouble/CreateInteger` secara lokal untuk field seperti `objHit`, `cause`, `attacker`, `orderGiver`, dan `anchorObj`.
  - Helper lokal sekarang memakai `CreateObjPointer(pObj->GetUniverse().GetCC(), pObj)` agar encoding referensi objek konsisten dengan guard 64-bit yang sudah dipasang di `GetObjPointerValue`, `CreateObjFromItem`, dan `AsSpaceObject`.
  - Ini secara khusus relevan dengan hit effects di `Transcendence/TransCore/CoreHitEffects.xml`, yang membaca `(@ gData 'objHit)` lalu memanggil `objGetProperty` selama `CWeaponFireDesc::CreateHitEffect` dijalankan di intro combat simulation.
- Verifikasi lokal pada slice ini sekarang mencakup rebuild dan validasi dari working directory non-repo melalui `lldb --batch` dari `/tmp`. Hasil yang terkonfirmasi:
  - `cmake --build --preset macos-debug --target transcendence_app -j8` tetap hijau;
  - `lldb --batch -o 'run' -o 'bt all' -- ./build/macos-debug/Transcendence` yang dijalankan dari `/tmp` mencapai `CLoadingSession::OnPaint first paint` dan bertahan sampai harness timeout 240 detik tanpa backtrace crash baru;
  - `build/macos-debug/Debug.log` untuk run yang sama kembali maju sampai `CIntroSession::Paint calling Render` tanpa `Crash in`, `EXC_BAD_ACCESS`, `CException:`, atau signature `CEffectGroupCreator` baru sebelum timeout.
- Audit tree untuk asset portability juga tidak menemukan jalur playback video eksplisit (`bink`/`theora`/`mp4`/`mov`/movie player) di source aktif, sehingga pekerjaan path asset macOS saat ini tetap terfokus pada image/resource roots dan soundtrack SDL_mixer, bukan backend video terpisah.
- Audit lanjutan pada jalur asset memperjelas pembagian portability yang sekarang aktif:
  - **Soundtrack music** (`Mammoth/TSUI/CMCIMixerStub.cpp`) sudah memakai `SDL_mixer` dan punya resolver path sendiri yang mencoba filespec asli, normalisasi slash, base path SDL, build tree, serta kandidat bundle `Contents/Resources/Game`.
  - **Gameplay/UI sound effects** (`Mammoth/TSE/CSoundResource.cpp` -> `CResourceDb::LoadSound` -> `Alchemy/DirectXUtil/CSoundMgrSDL.cpp`) sekarang sudah bebas dari `DirectSound` di jalur macOS, tetapi untuk file eksternal masih hanya mengandalkan `m_sRoot + sFolder + sFilename` lalu `Mix_LoadWAV(...)` tanpa fallback executable-relative/bundle-relative tambahan seperti soundtrack.
  - **Image loading** title/menu dan gameplay sudah melewati seam non-Windows (`CResourcePathResolver`, `CResourceDb::LoadImage`, `CResourceDb::LoadImageFile`, `CObjectImage`, `SDLBitmap`), termasuk normalisasi slash Windows-style pada path XML image resources.
  - **Video** masih tidak punya backend khusus di tree aktif, sehingga tidak ada blocker video terpisah yang perlu dikerjakan untuk milestone macOS saat ini.
- Gap portability yang semula terbuka dari audit source sebagian sudah ditutup di slice berikutnya pada `CSoundMgrSDL.cpp`:
  - resolver audio untuk **sound effects** kini ikut menormalisasi separator Windows dan mencoba fallback build-tree / bundle-adjacent (`Transcendence/Game`, `../Transcendence/Game`, `../../Transcendence/Game`, `Contents/Resources/Game`, serta kandidat berbasis `SDL_GetBasePath()`), sehingga parity lookup asset eksternal lebih dekat ke resolver soundtrack;
  - `CSoundMgrSDL::Play(...)` sekarang menerapkan pan melalui `Mix_SetPanning`, jadi parity spatial/stereo effect tidak lagi sepenuhnya hilang di macOS;
  - backend music SDL sekarang menyimpan filespec aktif, panjang track, posisi playback saat ini, dan `PlayMusic(..., iPos)` mencoba seek/resume berbasis `Mix_SetMusicPosition` bila codec mendukungnya.
- Gap yang masih tersisa setelah slice tersebut:
  - seek/resume music masih bergantung pada dukungan codec SDL_mixer, sehingga parity absolut dengan backend lama belum bisa dianggap final tanpa validasi runtime nyata;
  - validasi runtime yang lebih dalam untuk memastikan audio benar-benar terdengar/berperilaku benar pada run non-repo atau bundle belum selesai di slice ini;
  - audit asset tree saat ini hanya menemukan satu soundtrack shipped (`TranscendenceIntro.mp3`) dan tetap tidak menemukan backend atau asset video khusus, sehingga follow-up asset aktif tetap berada pada verifikasi audio nyata dan crash intro-effect yang lebih jarang.
- Karena itu blocker aktif belum dinyatakan selesai, tetapi sudah direklasifikasi: crash intro-effect yang sebelumnya direproduksi lewat backtrace kini paling mungkin berasal dari object-reference yang diberikan ke script hit-effect atau dari jalur runtime yang lebih jarang daripada smoke run batch saat ini. Di sisi asset, audit source tidak membuka blocker video baru, dan sebagian besar gap parity audio source-level sudah ditutup, tetapi verifikasi runtime audio nyata serta validasi manual visual/interaktif tetap tersisa.
