...
- macOS processor-count detection now has a native non-Windows path (`sysctl`/`sysconf`) instead of relying on Win32-only APIs during SFX thread planning.
- Intro viewport stabilization is now additionally guarded by forcing background painting single-threaded on macOS while still booting a valid background-paint thread pool object for callers that assume a non-NULL pool.
- A follow-up runtime slice also forces single-threaded object or sprite image painting on macOS via `SViewportPaintCtx::bForceSTPaint`, after a reproduced normal smoke-run crash advanced from `PaintViewport` into `PaintImage` with `CException: Out of memory.` during intro rendering.
...
- The latest Apple Silicon object-reference hardening slice also tightens several remaining script-facing paths so live `CSpaceObject *` references stay encoded as object-pointer atoms instead of legacy integer atoms. Additional guards in `GetObjPointerValue`, `CreateObjFromItem`, `AsSpaceObject`, and related property/global-definition helpers now reject low/truncated 64-bit values before dereference.
- A follow-up fix closes the same seam inside `CCreatePainterCtx`: hit-effect and anchor data fields such as `objHit`, `cause`, `attacker`, `orderGiver`, and `anchorObj` now use `CreateObjPointer(...)` instead of ad-hoc raw pointer numeric atoms. This specifically matches the `CoreHitEffects.xml` `GetParameters` path that reads `(@ gData 'objHit)` and immediately calls `objGetProperty` during `CWeaponFireDesc::CreateHitEffect`.
- The SDL-backed bitmap bridge has also been hardened for intro/menu asset compatibility: non-16-bit surfaces are normalized to 24-bit BGR where needed, `SDLBitmap` now reports real bit depth through `dibGetInfo`, and 24-bit/16-bit detection no longer aliases every RGB surface to 16-bit. This narrows one more portability seam for JPEG/BMP assets and alpha-mask consumers on macOS.
- Rebuild verification after the latest slice remains green (`cmake --build --preset macos-debug --target transcendence_app -j8`), and a batch `lldb` run launched from `/tmp` now reaches `CLoadingSession::OnPaint first paint` and sits in the intro loop until the 240-second harness timeout without emitting a new crash backtrace.
- The matching `build/macos-debug/Debug.log` still advances through intro init plus `CIntroSession::Paint calling Render`, which means non-repo cwd resource lookup remains functional for the loading/title assets and the previously reproduced effect crash is not triggered by every sustained batch run.
- Source audit for asset portability also found no active dedicated video-playback pipeline (`bink`, `theora`, `mp4`, `mov`, or movie-player code) in the current tree, so open asset-path work on macOS remains centered on image/resource resolution and soundtrack playback rather than a missing video backend.
- The asset audit also clarified that image loading is mostly on the right portability seam now (`CResourcePathResolver`, `CResourceDb`, `CObjectImage`, `SDLBitmap`), but audio is split across two different maturity levels:
  - soundtrack playback uses the macOS `SDL_mixer` bridge in `CMCIMixerStub.cpp` plus explicit path fallback candidates for build-tree and bundle-style layouts;
  - gameplay/UI sound effects already bypass `DirectSound` through `CSoundMgrSDL.cpp`, but external SFX file lookup still relies on `CResourceDb::LoadSound` composing `m_sRoot + sFolder + sFilename` with no soundtrack-style executable/bundle fallback sweep.
- The same audit initially found three remaining audio parity gaps that were not yet release-ready blockers for first render: external SFX path fallback, stereo pan support in `CSoundMgrSDL::Play(...)`, and missing music seek/play-state reporting (`PlayMusic(..., iPos)` and `GetMusicPlayState`).
- The latest follow-up slice closes most of that gap inside `CSoundMgrSDL.cpp`: external WAV loading now normalizes Windows separators and probes build-tree / bundle-adjacent candidates using `SDL_GetBasePath()`, effect playback now applies SDL_mixer panning, and music state now reports filename/length/current position while `PlayMusic(..., iPos)` attempts seek/resume when the codec supports it.
- Validation after that slice is still partial, but important: `cmake --build --preset macos-debug --target transcendence_app -j8` remains green, a non-repo `/tmp` launch still reaches `CIntroSession::Paint calling Render`, and the tree audit still shows only one shipped soundtrack asset (`TranscendenceIntro.mp3`) with no dedicated video-playback asset/backend in the current source tree.
- This means the prior intro viewport `Out of memory` report is no longer the best current blocker to chase; the newer high-value blocker remains effect-painter creation during longer-lived intro combat simulation, though the latest `CCreatePainterCtx` fix likely narrows that seam and needs interactive/runtime confirmation.
- Menu/input bridge code exists, but M4 now primarily needs an interactive manual validation pass for keyboard, mouse, wheel, text input, Retina behavior, sustained intro/menu rendering under real window interaction, plus deeper non-repo runtime checks for actual audio playback behavior under a real desktop session.
