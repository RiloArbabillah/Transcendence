...
- Local rebuild verification after that slice remains green (`cmake --build --preset macos-debug --target transcendence_app -j8` -> `ninja: no work to do`), and the most recent `build/macos-debug/Debug.log` still shows startup progressing through intro init and `CIntroSession::Paint calling Render` before `Initializing adventure: ../../Transcendence/TransCore/Transcendence.xml`.
- This means the prior intro viewport `Out of memory` report is no longer the best current blocker to chase; the newer high-value blocker is still effect-painter creation during longer-lived intro combat simulation.
